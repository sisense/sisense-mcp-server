...
describe('Get Data Source Fields Tool E2E', () => {
  beforeAll(async () => {
    await setupE2ETests();
  });

  it(
    'should call get-data-source-fields tool via MCP client',
    async () => {
      // Dynamically import tool name after browser mock is initialized
      const { TOOL_NAME_GET_DATA_SOURCE_FIELDS } = await import('@sisense/sdk-ai-core');
